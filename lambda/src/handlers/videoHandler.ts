...
export async function handleGenerateVideo(event: any, headers: any): Promise<APIGatewayProxyResult> {
  try {
    ...
    if (provider === 'google-veo') {
      ...
      // We route 'kling' provider to Fal AI to avoid direct Kling API balance issues
      console.log('🎯 [VideoGen] Routing to Fal AI provider (provider:', provider, ')');
      
      // Generate video with Fal AI
      const videoRequest: any = {
        model: model as 'kling-v2.5-turbo-pro' | 'kling-v2.5-turbo-standard' | 'kling-v1-5-pro',
        prompt,
        negativePrompt,
        imageUrl: finalImageUrl,
      ...
      // Default to Kling AI (deprecated)
      console.log('⚠️ [VideoGen] WARNING: Using deprecated Kling direct API');
      console.log('🎯 [VideoGen] Routing to Kling provider (direct API)');
...
export async function handleVideoStatus(event: any, headers: any): Promise<APIGatewayProxyResult> {
  try {
    ...
    if (provider === 'google-veo') {
      ...
      // Route both fal-ai and kling to Fal AI status check
      result = await checkFalStatus(taskId);
    } else {
